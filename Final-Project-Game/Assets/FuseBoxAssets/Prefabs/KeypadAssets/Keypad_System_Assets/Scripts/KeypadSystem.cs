...
//    public DoorScript doorScript;
    private int[] donnaKeypadSolution = new int[] {3, 4, 7, 7, 9, 2, 2};
    private int[] terryKeypadSolution = new int[] {6, 1, 7, 5, 9, 9, 2};
    ...
    private void CheckKeypadPuzzleSolution()
    {
        int i = 0;
        foreach (int number in numbers)
        {
            if (number == terryKeypadSolution[i] || number == donnaKeypadSolution[i])
            {
                // Puzzle solved - Do whatever here
            }
            i++;
        }
    }
